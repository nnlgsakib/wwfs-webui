...
function webpackOverride (config) {
  ...
  // Suppress source map warnings from dag-jose package
  config.ignoreWarnings = [
    ...(config.ignoreWarnings || []),
    /** @param {import('webpack').WebpackError & { module?: { resource?: string }, message?: string }} warning */
    (warning) => {
      const msg = warning && (warning.message || '')
      const res = warning && warning.module && warning.module.resource
      return (typeof msg === 'string' && msg.includes('dag-jose/src')) ||
             (typeof res === 'string' && res.includes('node_modules/dag-jose'))
    }
  ]
